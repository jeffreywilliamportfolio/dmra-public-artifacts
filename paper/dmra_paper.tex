...
The audit uses 66 unique matched prompt pairs and 328 run/prompt
records. Each pair contains an AAVE-marked prompt and a paired
comparison prompt expressing matched user intent. The unique-pair count
is 66; the larger record count reflects repeated observations over those
pairs across model variants and answer-only or visible-reasoning
templates.
...
The model hashes used in the sensitive-prompt setup are:
...
\tightlist
\item
  200 records from 50 everyday/register pairs across base and Hauhau
  answer-only runs;
\item
  40 records from 5 medical-triage pairs across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  48 records from 6 chest-pain triage pairs across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  8 records from 1 financial-stress pair across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  32 records from 4 sensitive safety pairs across base/Hauhau and
  answer-only/visible-reasoning runs.
\end{itemize}
...
size \texttt{4096}. Model weights and environment files are outside the
manuscript package. Verification metadata and archive-scope notes are
preserved with the artifact materials.

\section{Results}\label{results}

The initial 50-pair answer-only run completed all prompt artifacts.
...
Pair \texttt{003} in the initial base answer-only run asks how to
interpret the model's answer when asked whether it has beliefs. The
...
Medical triage prompts show a different pattern. In several emergency
...
Sensitive safety runs show how refusal-reduced models can expose
