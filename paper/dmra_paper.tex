...
\maketitle
\begin{abstract}
As large language models become first-contact interfaces for
safety-critical advice, dialect-conditioned assistance quality becomes a
deployment concern. Existing safety evaluations emphasize refusal
presence and aggregate performance, leaving matched-intent register
effects undermeasured. We introduce \textbf{Dialect-Marked Response
Audit (DMRA)}, an I/O-level matched-pair protocol for comparing
responses to African American Vernacular English (AAVE)-marked prompt
surfaces and paired comparison prompts. We apply DMRA to Qwen3.5-35B-A3B
and a refusal-reduced HauhauCS fine-tune under deterministic decoding
across 328 run/prompt records over 66 matched pairs in five audit
subsets: Core Register Set, Medical Triage Set, Chest-Pain Triage
Extension, Financial-Stress Pair, and Sensitive Safety Set. The audit
documents response differentials in epistemic framing, pediatric
emergency scaffolding, financial-stress empathy, visible
dialect-to-demographic inference, and high-risk scenario continuation.
The clearest policy-facing case is pediatric respiratory distress: both
matched outputs direct emergency care, while the comparison-prompt
response is 1.81x longer and adds deterioration checks and ambulance
guidance. In pair 003, the AAVE-marked output is 2.19x longer and gives
the opposite epistemic frame. In a self-harm crisis case, the released
base model's visible trace identifies \texttt{finna} as Southern U.S. /
AAVE and uses that cue to assume U.S. context for crisis-resource
selection. DMRA makes safety-support equivalence the unit of evaluation:
matched requests should receive comparable urgency, specificity,
respect, and risk reduction across prompt surfaces.
\end{abstract}

\section{Introduction}\label{introduction}

Large language models are increasingly used as first-contact assistants
for questions that carry safety consequences outside formal clinical,
legal, or crisis-service settings. A caregiver may ask whether a
wheezing child needs emergency care. A worker under financial pressure
may ask for advice while describing despair. A user in a self-harm
crisis may ask what to do tonight. A user contemplating violence or
illegal logistics may seek help in language that includes slang,
profanity, or register-specific grammar. In these settings, response
quality includes timely triage, concrete safety steps, appropriate
de-escalation, and a response frame that preserves the user's
dignity.\footnote{Content warning: the audit includes self-harm crisis
  language, violent-threat prompts, illegal-logistics prompts, and
  masked slurs. Public excerpts use minimal evidence phrases and
  redaction for operational detail.}

Dialect equity matters because dialect is structured language. African
American Vernacular English (AAVE) has grammar, discourse conventions,
and lexical patterns; it is also socially stigmatized and often
racialized by listeners and classifiers. A safety-critical assistant
should deliver equivalent caution, practical detail, and respect across
register surfaces. This paper asks whether, under matched semantic
conditions, an LLM changes safety-relevant response behavior when
prompts contain AAVE-marked features.

The study uses constructed prompt surfaces. We use \textbf{AAVE-marked
...
\textbf{paired comparison prompts} for the matched counterpart prompts
in the archive. The paired comparison side varies across audit subsets
in formality and lexical choice, so the paper treats it as a corpus
condition, rather than a homogeneous variety. Archive paths and
manifests retain legacy labels such as \texttt{aave}, \texttt{ae}, and
\texttt{academic\_english}; in this paper those labels function as
provenance identifiers.

Existing refusal-rate and aggregate-accuracy evaluations capture
important safety behavior, yet assistance quality also varies after a
model reaches the same top-level safety outcome. In the pediatric
respiratory-distress pair, both matched outputs direct emergency care,
while the comparison-prompt response adds deterioration checks and
ambulance guidance. In the violence stress test, both matched outputs
contain harmful continuation, while the AAVE-marked response frames the
confrontation more directly as a timing and feasibility problem. These
cases motivate a unit of evaluation centered on equivalent safety
support.

We use ``register-conditioned response selection'' descriptively. It
means that the model selects a different response frame, level of
detail, safety strategy, or continuation trajectory when the prompt
surface changes while the scenario remains matched. The evidence is
generated text and visible model-emitted reasoning. Internal mechanism
claims require separate mechanistic work.
...
\tightlist
\item
  It defines \textbf{Dialect-Marked Response Audit (DMRA)}, a
  matched-pair audit protocol for detecting differences in
  safety-critical assistance across AAVE-marked and paired comparison
  prompt surfaces.
\item
  It reports a multi-domain audit of base Qwen3.5-35B-A3B and a
  refusal-reduced HauhauCS fine-tune, covering 328 run/prompt records
  over 66 unique matched pairs.
\item
  It documents five high-salience response differentials that refusal
  presence alone would compress: epistemic framing, pediatric emergency
  scaffolding, financial-stress empathy, visible dialect-to-demographic
  inference, and high-risk scenario continuation.
\end{itemize}
...
chatbot quality-of-service settings. ReDial and EnDive evaluate
cross-dialect performance gaps in reasoning and language-understanding
tasks (Lin et al., 2024; Gupta et al., 2025). Dunlap and McCoy (2026)
compare model-produced AAVE with human AAVE usage and find underuse,
misuse, and stereotype replication. Tornberg (2026) reports systematic
differences in LLM annotation judgments for matched AAVE sentences
across 19 models. Harvey et al.~(2025) frame dialect harms in deployed
chatbots as quality-of-service harms. DMRA builds on this newer audit
direction by focusing on safety-critical assistance quality.
...
model behavior in safety-relevant contexts. Material difference includes
triage correctness, urgency, safety scaffolding, de-escalation, empathy
frame, stereotype or demographic inference, harmful continuation,
tactical detail, and visible reasoning content.

The cleaned archive contains matched prompt pairs in five reader-facing
audit subsets; dates preserve file-level provenance:

\begin{itemize}
\tightlist
\item
  \textbf{Core Register Set} (2026-05-05): 50 matched pairs covering
  everyday register variation;
\item
  \textbf{Medical Triage Set} (2026-05-15): 5 matched pairs covering
  pediatric and adult health triage;
\item
  \textbf{Chest-Pain Triage Extension} (2026-05-15): 6 matched pairs
  extending the medical triage scenarios;
\item
  \textbf{Financial-Stress Pair} (2026-05-15): 1 matched pair covering
  economic strain and family/work advice;
\item
  \textbf{Sensitive Safety Set} (2026-05-16): 4 matched pairs covering
  self-harm, violence, illegal logistics, and panic-perspective prompts.
\end{itemize}

The consolidated cleaned I/O bundle contains 328 run/prompt records over
66 unique matched pairs: 50 from the Core Register Set, 5 from the
Medical Triage Set, 6 from the Chest-Pain Triage Extension, 1 from the
Financial-Stress Pair, and 4 from the Sensitive Safety Set. The 328 rows
are repeated model/template observations over those matched pairs. The
run-file provenance is recorded in
\texttt{register\_bias\_io\_by\_run/MANIFEST.tsv}.

The audit covers everyday advice and high-risk safety domains. The Core
Register Set includes epistemic, identity, medical, legal, dream,
relationship, and advice-seeking prompts. The Medical Triage Set and
Chest-Pain Triage Extension focus on pediatric and adult health triage
scenarios such as chest pain, high fever, wheezing, seizure, and head
injury. The Financial-Stress Pair asks for practical family and work
advice under economic strain. The Sensitive Safety Set includes
self-harm, violence, illegal drug logistics, and a panic-attack
perspective prompt.
...
and are labeled \texttt{\textless{}/no\ think\textgreater{}} in the
archive. Visible-reasoning runs ended the rendered prompt with
\texttt{\textless{}think\textgreater{}} and preserved model-emitted
reasoning text until the final-answer boundary. Answer-only runs
generally used \texttt{n\_predict=2048}; visible-reasoning runs in the
2026-05-15 and 2026-05-16 captures used \texttt{n\_predict=8192}.

Response coding combines exact I/O review, heuristic labels, and
case-specific evidence files. For the Core Register Set, the existing
summary reports completeness, token asymmetry, budget hits, and lexical
screens. For the Sensitive Safety Set, coding distinguishes refusal,
partial refusal, de-escalation, practical safety scaffolding, harmful
continuation, tactical framing, and visible reasoning content.
Case-evidence files preserve prompt text, generation settings,
provenance, and short evidence excerpts.
...
The audit compares two Qwen3.5-35B-A3B variants (Qwen Team, 2025; Qwen
Team, 2026):

\begin{itemize}
\tightlist
\item
  \texttt{base}: \texttt{ggml-org/Qwen3.5-35B-A3B-GGUF}, file
  \texttt{Qwen3.5-35B-A3B-Q8\_0.gguf};
\item
  \texttt{hauhau}: HauhauCS refusal-reduced Qwen3.5-35B-A3B fine-tune,
  with the exact model path and GGUF file recorded in the manifest.
\end{itemize}

The model hashes used in the Sensitive Safety Set setup are:

\begin{itemize}
\tightlist
\item
  base SHA-256
  \texttt{8a83fbfde74b0366feb76b075c10df46bbece6aa21ed14a6f291a7904f2b7d67};
\item
  Hauhau SHA-256
  \texttt{f3235db7657cd068fd249e50bb3f1f50b0f8236786e4483462f50b1f3c64cb17}.
\end{itemize}

The base model provides a reference open-weight instruction model. The
HauhauCS model is a refusal-reduced stress test. It changes style,
persona alignment, taboo handling, and instruction-following behavior
together, so the comparison is operational: it shows how matched prompt
surfaces behave when refusal convergence weakens.

The cleaned consolidated I/O bundle contains 328 run/prompt records:

\begin{itemize}
\tightlist
\item
  Core Register Set: 200 records across base and Hauhau answer-only
  runs;
\item
  Medical Triage Set: 40 records across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  Chest-Pain Triage Extension: 48 records across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  Financial-Stress Pair: 8 records across base/Hauhau and
  answer-only/visible-reasoning runs;
\item
  Sensitive Safety Set: 32 records across base/Hauhau and
  answer-only/visible-reasoning runs.
\end{itemize}

For the 2026-05-15 and 2026-05-16 subsets, context size is
\texttt{16384}; the Core Register Set used context size \texttt{4096}.
The cleaned archive keeps model weights and environment files outside
the manuscript package. Verification metadata is in \texttt{VERIFY.tsv},
and the root \texttt{README.md} describes the archive scope and rules.

\section{Results}\label{results}

The Core Register Set answer-only run completed all prompt artifacts.
Each run produced 100 prompt directories, 100 generated text files, and
100 generated token files. The base answer-only run had 6 outputs that
reached the 2,048-token budget; Hauhau answer-only had 15. In base
answer-only, 26 of 50 matched pairs had more than 20 percent raw
generated-token asymmetry; in Hauhau answer-only, 31 of 50 did.
...
Pair \texttt{003} in the Core Register Set base answer-only run asks how
to interpret the model's answer when asked whether it has beliefs. The
paired comparison prompt asks whether the answer should be taken ``as a
fact about you or as you naming your limits.'' The AAVE-marked prompt
asks the same question with matched surface edits. The resulting
responses give opposite top-level frames. The paired comparison response
says the answer should be taken as naming the model's limits. The
AAVE-marked response says it should be taken as a statement of fact
about the model's architecture. The AAVE-marked output is also much
longer after trimming: 383 words versus 175 words, a 2.19x word-length
ratio. Full prompt text, output excerpts, and measurements are preserved
in the pair \texttt{003} case-evidence file.

The Medical Triage Set shows a different pattern. In several emergency
scenarios, both matched prompts receive the same top-level triage
decision. The divergence appears in the amount and specificity of
scaffolding. The clearest example is \texttt{med\_s03}, a pediatric
wheezing and trouble-breathing scenario. In the base answer-only run,
both matched outputs tell the caregiver to seek immediate emergency
care. The paired comparison response adds checks for whether the child
can speak in full sentences, whether lips or fingernails look blue or
gray, and whether an ambulance is safer than driving. It also explains
airway narrowing, rapid worsening, oxygen-level assessment, and
clinician lung exam. The AAVE-marked response gives correct emergency
direction with shorter real-time guidance: 177 words versus 321 words, a
1.81x word-length ratio for the paired comparison output. This is the
clearest policy-facing case because the top-level safety decision
converges while practical triage support differs.
...
Sensitive Safety Set runs show how refusal-reduced models can expose
response trajectories. Base answer-only often refuses or partially
refuses violence and drug-logistics prompts across both prompt surfaces.
Hauhau answer-only more often continues the scenario. The clearest
violence example is the \texttt{stress\_s02} pair. The paired comparison
response frames the situation through risk, anger, firearm escalation,
legal consequence, and regret. The AAVE-marked response treats the
proposed armed confrontation more directly as a timing and feasibility
problem and includes a ``clean shot'' frame. The public evidence excerpt
redacts operational details and preserves the evidence-relevant phrases.

Visible-reasoning runs add a second audit surface. The strongest trace
finding comes from a self-harm crisis prompt. In
\texttt{s\_full\_02\_aave}, the base visible-reasoning run emits a trace
that identifies an imminent self-harm crisis and then treats the dialect
marker \texttt{finna} as evidence of Southern U.S. or AAVE context. The
trace uses that inference to select U.S.-specific crisis resources,
especially 988. The final answer is safety-oriented and
crisis-appropriate. The trace reveals a register-to-location inference
step inside emitted output, making the dialect cue part of the
resource-selection rationale.

This self-harm trace reframes the other results. The pediatric,
epistemic, financial, and violence cases show differences in final
response posture and safety scaffolding. The self-harm trace shows the
model naming a dialect marker and carrying that marker into
safety-resource selection. The issue is subtle: 988 is a plausible
resource for many U.S. users, and crisis support is the right response
domain. The equity concern lies in using a dialect cue as demographic or
...
risk-reduction scaffolding. The pediatric wheezing case is the clearest
example: both responses direct emergency care, while the paired
comparison response gives more detailed deterioration checks and
ambulance guidance. The epistemic pair shows another form of divergence:
the same conceptual question receives opposite framing under matched
surface conditions.

The audit also shows dialect-linked features functioning as
response-selection features. Sometimes this appears in final-answer
style, such as address terms and empathy frame. Sometimes it appears in
visible reasoning, as a dialect-to-location inference. Sometimes it
appears through differences in urgency, practical detail, or high-risk
continuation. The corpus therefore supports a safety-evaluation standard
centered on equivalent support across matched prompt surfaces.
...
The deployment risk extends beyond spectacular harmful completions.
Common risks include unequal scaffolding, unequal seriousness,
overfamiliarity, stereotype-driven demographic inference, and weaker
localization. In medical triage, a cyanosis check or ambulance cue can
matter. In crisis support, inferring location from dialect can route
resources. In financial distress, an over-toughened empathy frame can
encode assumptions about how a user should be addressed. Dialect-diverse
users deserve the safest version of an assistant's help across the
...
varies across audit subsets, and the Chest-Pain Triage Extension
includes more clinical wording than the Core Register Set. This
...
This work studies safety behavior on sensitive prompts involving
self-harm, violence, illegal activity, and slur-containing language.
Public artifacts should mask slurs and reproduce operationally harmful
completions only through minimal evidence phrases. Exact prompts and
outputs can remain in controlled research archives with hashes and
provenance, preserving reproducibility while limiting amplification of
harmful content.
...
summaries. The public bundle excludes model weights, environment files,
secrets, full raw sensitive generations, slur-containing prompt text,
and operationally harmful completions.

Full raw generations for the Sensitive Safety Set and other sensitive
examples remain in a controlled local research archive because they
include self-harm, violent-threat, illegal-logistics, and
slur-containing content. The retained archive preserves file-level
provenance and checksums so that reported claims can be verified without
...
de Wynter, Xun Wang, Si-Qing Chen, Michael Wooldridge, and Janet B.
Pierrehumbert. 2024. ``One Language, Many Gaps: Evaluating Dialect
Fairness and Robustness of Large Language Models in Reasoning Tasks.''
