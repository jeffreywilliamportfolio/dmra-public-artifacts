...
safety-critical advice, dialect-conditioned response quality becomes a
deployment concern. Standard LLM safety evaluations often emphasize
refusals and aggregate performance, leaving safety-support equivalence
across matched users undermeasured. We introduce
\textbf{Dialect-Marked Response Audit (DMRA)}, an I/O-level matched-pair
protocol for evaluating whether semantically matched prompts receive
equivalent safety support across African American Vernacular English
(AAVE)-marked and paired comparison prompt surfaces. We apply DMRA to
Qwen3.5-35B-A3B and a refusal-reduced HauhauCS fine-tune under
deterministic decoding across 328 run/prompt records over 66 matched
pairs spanning experiential, medical triage, chest-pain triage,
financial stress, and sensitive safety scenarios. The audit identifies
safety-relevant response differentials in epistemic framing, pediatric
emergency scaffolding, empathy style, visible dialect-to-demographic
inference, and high-risk continuation. A pediatric respiratory-distress
case anchors the policy risk: both matched outputs direct emergency
care, while the comparison-prompt response adds deterioration checks and
ambulance guidance compared with the AAVE-marked response. In an
epistemic prompt, matched surfaces flip the model's account of
belief-language from limit disclosure to architecture statement. In a
...
crisis-resource selection. DMRA reframes safety evaluation around
safety-support equivalence:
...
marker \texttt{finna} (a near-future marker meaning ``going to'') as
evidence of Southern U.S. or AAVE context. The trace uses that inference
to select U.S.-specific crisis resources,
