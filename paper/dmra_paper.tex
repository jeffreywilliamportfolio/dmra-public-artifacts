...
base model's visible trace identifies \texttt{finna} as Southern
U.S./AAVE and uses that cue to assume U.S. context for crisis-resource
...
The audit archive contains matched prompt pairs in five reader-facing
...
The consolidated I/O bundle contains 328 run/prompt records over 66
unique matched pairs: 50 from the Core Register Set, 5 from the Medical
Triage Set, 6 from the Chest-Pain Triage Extension, 1 from the
Financial-Stress Pair, and 4 from the Sensitive Safety Set. The 328 rows
are repeated model/template observations over those matched pairs.
Public run-file provenance is recorded in
\texttt{metadata/run\_manifest\_public.tsv}; retained full manifests are
preserved in the controlled archive.
...
case-specific evidence files. For the Core Register Set, summary files
report completeness, token asymmetry, budget hits, and lexical screens.
For the Sensitive Safety Set, coding distinguishes refusal, partial
refusal, de-escalation, practical safety scaffolding, harmful
continuation, tactical framing, and visible reasoning content. Public
case-evidence summaries are redacted for safety; retained case-evidence
files preserve prompt text, generation settings, provenance, and short
evidence excerpts.
...
The consolidated I/O bundle contains 328 run/prompt records:
...
Model weights and environment files are outside the manuscript package.
Verification metadata and archive-scope notes are preserved with the
artifact materials.
...
ratio. The retained pair \texttt{003} case-evidence file preserves full
prompt text, output excerpts, and measurements.
...
The author used OpenAI language-model tools for drafting assistance,
copyediting, LaTeX preparation, artifact checks, and reference-scope
review. The author reviewed the final manuscript and takes
responsibility for all claims, citations, analyses, and text.
